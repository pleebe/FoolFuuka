            ...
            <div class="thread_image_box" title="<?= $p->getCommentSanitized() ? htmlspecialchars(strip_tags($p->getCommentSanitized())) : '' ?>">
                ...
                <div class="post_file_controls">
                    <a href="<?= ($p_media->getMediaLink($this->getRequest())) ? $p_media->getMediaLink($this->getRequest()) : $p_media->getRemoteMediaLink($this->getRequest()) ?>" class="btnr" target="_blank">Full</a><a
                        ...
                        target="_blank" href="https://lens.google.com/uploadbyurl?url=<?= $p_media->getThumbLink($this->getRequest()) ?>" class="btnr parent">Google</a><?php if (!$p->radix->archive || $p->radix->getValue('archive_full_images')) : ?><a
