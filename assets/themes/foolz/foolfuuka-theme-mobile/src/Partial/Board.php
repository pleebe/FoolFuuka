        ...
        <article id="<?= $num ?>" class="clearfix thread doc_id_<?= $op->doc_id ?> board_<?= $op->radix->shortname ?>" data-doc-id="<?= $op->doc_id ?>" data-thread-num="<?= $op->thread_num ?>">
                    ...
                    <div class="post_file_controls">
                        ...
                        <?php if ( !$op->radix->hide_thumbnails || $this->getAuth()->hasAccess('maccess.mod')) : ?>
                            <a href="<?= $this->getUri()->create($op->radix->shortname . '/search/image/' . $op_media->getSafeMediaHash()) ?>" class="btnr parent"><?= _i('View Same') ?></a><a
                                href="http://www.google.com/searchbyimage?image_url=<?= $op_media->getThumbLink($this->getRequest()) ?>" target="_blank"
                ...
                <a href="<?= $this->getUri()->create(array($op->radix->shortname, 'thread', $num)) ?>" class="btnr parent"><?= _i('View') ?></a><a href="<?= $this->getUri()->create(array($op->radix->shortname, $controller_method, $num)) . '#reply' ?>" class="btnr parent"><?= _i('Reply') ?></a><?= (isset($post['omitted']) && $post['omitted'] > 50) ? '<a href="' . $this->getUri()->create($op->radix->shortname . '/last/50/' . $num) . '" class="btnr parent">' . _i('Last 50') . '</a>' : '' ?><?= ($op->radix->archive) ? '<a href="//boards.4chan.org/' . $op->radix->shortname . '/thread/' . $num . '" class="btnr parent">' . _i('Original') . '</a>' : '' ?><a href="#" class="btnr parent" data-post="<?= $op->doc_id ?>" data-post-id="<?= $num ?>" data-board="<?= htmlspecialchars($op->radix->shortname) ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true" data-function="report"><?= _i('Report') ?></a><?php if ($this->getAuth()->hasAccess('maccess.mod') || !$op->radix->archive) : ?><a href="#" class="btnr parent" data-post="<?= $op->doc_id ?>" data-post-id="<?= $num ?>" data-board="<?= htmlspecialchars($op->radix->shortname) ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true" data-function="delete"><?= _i('Delete') ?></a><?php endif; ?>
                        ...
                        <div class="btn-group post_mod_controls" style="clear:both; padding:5px 0 0 0;">
                            <button class="btn btn-mini" data-post="<?= $op->doc_id ?>" data-post-id="<?= $num ?>" data-board="<?= htmlspecialchars($op->radix->shortname) ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true" data-function="editPost"><?= _i('Edit Post') ?></button>
                            ...
                            <?php endif; ?>
                            <?php if ($op->poster_ip) : ?>
                            <button class="btn btn-mini" data-function="mod" data-board="<?= $op->radix->shortname ?>" data-ip="<?= Inet::dtop($op->poster_ip) ?>" data-action="delete_user"><?= _i('Delete All Post By IP') ?></button>
                ...
                <?php foreach ($op->getReports() as $report) : ?>
                    <div class="report_reason"><?= '<strong>' . _i('Reported Reason:') . '</strong> ' . $report->getReasonProcessed() ?>
                        ...
                        <div class="ip_reporter">
                            ...
                            <div class="btn-group">
                                <button class="btn btn-mini" data-function="ban" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true" data-board="<?= $op->radix->shortname ?>" data-ip="<?= $reportip ?>" data-action="ban_user"><?= _i('Ban IP:') . ' ' . $reportip ?></button>
                                <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-action="delete_report"><?= _i('Delete Report') ?></button>
                                <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-action="delete_all_report"><?= _i('Delete All Reports') ?></button>
                            </div>
                            ...
                            <div class="post_extra_mod" style="clear:both;">
                                <div class="btn-group">
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-action="delete_all_report_posts"><?= _i('Delete All Reported Posts') ?></button>
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-action="delete_all_report_image"><?= _i('Delete All Reported Images') ?></button>
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-action="ban_all_report_image"><?= _i('Ban All Reported Images') ?></button>
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-action="ban_global_all_report_image"><?= _i('Globally Ban All Reported Images') ?></button>
                                </div>
                                <div class="btn-group">
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-ip="<?= $reportip ?>" data-action="delete_all_ip_report"><?= _i('Delete All Reports From This IP') ?></button>
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-ip="<?= $reportip ?>" data-action="delete_all_report_posts_ip"><?= _i('Delete All Posts Reported By This IP') ?></button>
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-ip="<?= $reportip ?>" data-action="delete_all_report_image_ip"><?= _i('Delete All Images Reported By This IP') ?></button>
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-ip="<?= $reportip ?>" data-action="ban_all_report_image_ip"><?= _i('Ban All Images Reported By This IP') ?></button>
                                    <button class="btn btn-mini" data-function="mod" data-id="<?= $report->id ?>" data-board="<?= $op->radix->shortname ?>" data-ip="<?= $reportip ?>" data-action="ban_global_all_report_image_ip"><?= _i('Globally Ban All Images Reported By This IP') ?></button>
                                </div>
                            </div>
                        </div>
                    </div>
